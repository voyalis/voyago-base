...
import (
	...
	"encoding/hex"
	"fmt"
	"log/slog" // slog kullanıyoruz

	// "os" // Kaldırıldı, burada kullanılmıyor
	"strings"
	"time"
	...
	"github.com/voyalis/voyago-base/src/authservice/repository" // EKLENDİ
)

// UserRepository interface'i, AuthService'in repository'ye olan bağımlılığını tanımlar.
type UserRepository interface {
	CreateUser(ctx context.Context, email, passwordHash, fullName string) (*repository.User, error) // repository.User tipini kullanıyoruz
	...
	GetRefreshTokenByHash(ctx context.Context, tokenHash string) (*repository.RefreshToken, error) // repository.RefreshToken tipini kullanıyoruz
...
func NewAuthServiceServer(repo UserRepository, secretKeyVal string) *AuthServiceServer {
	if secretKeyVal == "" {
		...
		// Bu kritik bir hata, uygulamanın başlamaması gerekir.
...
func (s *AuthServiceServer) Register(ctx context.Context, req *pb.RegisterRequest) (*pb.RegisterResponse, error) {
	...
	// CreateUser'dan dönen *repository.User'ı *pb.UserInfo'ye çeviriyoruz.
	// Varsayılan rollerin CreateUser içinde atandığını varsayıyoruz ve GetUserByID ile teyit ediyoruz.
	...
	if err != nil {
		...
		// Fallback: En azından ID, email, name ile dönelim, roller varsayılan olsun.
		userInfo = &pb.UserInfo{
			UserId:        createdRepoUser.ID.String(),
			Email:         createdRepoUser.Email,
			FullName:      createdRepoUser.FullName.String,
			IsActive:      createdRepoUser.IsActive,
			EmailVerified: createdRepoUser.EmailVerified,
			Roles:         []string{"passenger"}, // Güvenli varsayım
		}
	}
...
func (s *AuthServiceServer) Login(ctx context.Context, req *pb.LoginRequest) (*pb.LoginResponse, error) {
	...
	if err != nil {
		slog.WarnContext(ctx, "Service: Login failed - user not found or DB error during GetUserByEmail", "email", req.Email, "error", err.Error())
		return nil, status.Errorf(codes.Unauthenticated, "Invalid email or password")
	}

	if !isActive { // Bu kontrol UserInfo.IsActive üzerinden de yapılabilir, ama repository zaten isActive dönüyor.
	...
	accessClaims := jwt.MapClaims{
		...
		"exp": accessTokenExp.Unix(), "iat": time.Now().Unix(),
	}
	...
	if err := s.userRepo.StoreRefreshToken(ctx, uuid.MustParse(userInfo.UserId), hashedRefreshToken, refreshTokenExp); err != nil {
		...
		// Bu hatayı logluyoruz ama login işlemini engellemiyoruz. Üretimde daha dikkatli ele alınabilir.
	}

	if err := s.userRepo.UpdateUserLastSignInAt(ctx, uuid.MustParse(userInfo.UserId)); err != nil {
	...
	return &pb.LoginResponse{
		User:         userInfo, AccessToken: signedAccessToken, RefreshToken: rawRefreshToken,
...
func (s *AuthServiceServer) ValidateToken(ctx context.Context, req *pb.ValidateTokenRequest) (*pb.ValidateTokenResponse, error) {
	...
	return nil, status.Errorf(codes.Unauthenticated, "Invalid access token structure or content")
}
...
func (s *AuthServiceServer) RefreshAccessToken(ctx context.Context, req *pb.RefreshAccessTokenRequest) (*pb.LoginResponse, error) {
	...
	if err != nil {
		slog.WarnContext(ctx, "Service: Invalid or expired refresh token", "error", err.Error(), "provided_token_hash_prefix", hashedRefreshToken[:min(8,len(hashedRefreshToken))])
		return nil, status.Errorf(codes.Unauthenticated, "Invalid or expired refresh token")
	}

	// Eski refresh token'ı revoke et (Token Rotasyonu)
	if err := s.userRepo.RevokeRefreshTokenByHash(ctx, dbToken.TokenHash); err != nil {
		slog.ErrorContext(ctx, "Service: Failed to revoke old refresh token during rotation", "userID", dbToken.UserID.String(), "tokenHash_prefix", dbToken.TokenHash[:min(8,len(dbToken.TokenHash))], "error", err.Error())
	...
	if err != nil || (userInfo != nil && !userInfo.IsActive) { // userInfo nil değilse IsActive kontrolü
		slog.ErrorContext(ctx, "Service: User for refresh token not found or not active", "userID", dbToken.UserID.String(), "error", err)
		return nil, status.Errorf(codes.Unauthenticated, "User associated with refresh token is invalid")
	}
	if userInfo == nil && err == nil { // Beklenmedik durum, GetUserByID nil user ama nil error dönmemeli
		slog.ErrorContext(ctx, "Service: GetUserByID returned nil user and nil error for refresh token", "userID", dbToken.UserID.String())
		return nil, status.Errorf(codes.Internal, "Internal server error during refresh token processing")
	}
	...
	accessToken := jwt.NewWithClaims(jwt.SigningMethodHS256, accessClaims)
	signedAccessToken, err := accessToken.SignedString(s.jwtSecret) // err shadowingi olmaması için yeni değişken
	if err != nil { 
		slog.ErrorContext(ctx, "Service: Error signing new access token during refresh", "userID", userInfo.UserId, "error", err.Error())
	...
	if errStore := s.userRepo.StoreRefreshToken(ctx, dbToken.UserID, newHashedRefreshToken, newRefreshTokenExp); errStore != nil { // errStore yeni değişken
	...
	return &pb.LoginResponse{
		User: userInfo, AccessToken: signedAccessToken, RefreshToken: newRawRefreshToken,
		ExpiresIn: int32(time.Until(accessTokenExp).Seconds()),
	}, nil
}
...
func (s *AuthServiceServer) Logout(ctx context.Context, req *pb.LogoutRequest) (*pb.LogoutResponse, error) {
	...
	if req.RefreshToken == "" {
		slog.InfoContext(ctx, "Service: Logout successful (client-side access token invalidation only, no refresh token provided)")
		return &pb.LogoutResponse{Message: "Logged out. No server-side refresh token to revoke."}, nil
	}

	hashedRefreshToken := hashRefreshToken(req.RefreshToken)
	err := s.userRepo.RevokeRefreshTokenByHash(ctx, hashedRefreshToken)
	if err != nil {
		slog.ErrorContext(ctx, "Service: Error revoking refresh token during logout", "error", err.Error(), "tokenHash_prefix", hashedRefreshToken[:min(8,len(hashedRefreshToken))])
		return &pb.LogoutResponse{Message: "Logout processed, but server-side token revocation might have failed."}, nil
	}
...
// Diğer RPC'ler için Unimplemented stub'ları:
